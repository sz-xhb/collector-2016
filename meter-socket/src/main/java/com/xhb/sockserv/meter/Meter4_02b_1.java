...
public class Meter4_02b_1 extends Meter4_02 {

	public Meter4_02b_1(ReceiptCollector receiptCollector, ReceiptMeter receiptMeter) {
	...
	public void handleResult() {
		...
		for (int i = 0; i < 3 && i < receiptCircuitList.size(); i++) {
		...
		for (int i = 9; i < 12 && i - 6 < receiptCircuitList.size(); i++) {
			if (config.isRateFromDataBaseEnabled()) {
				if (receiptCircuitList.get(i-6) == null) {
					return;
				}
				if (receiptCircuitList.get(i-6).getVoltageRatio() != null) {
					Ubb = receiptCircuitList.get(i-6).getVoltageRatio();
				}
				if (receiptCircuitList.get(i-6).getCurrentRatio() != null) {
					Ibb = receiptCircuitList.get(i-6).getCurrentRatio();
				}
			}
			resultMutiplyRate(i);
			DataElectricity dataElectricity = new DataElectricity();
			dataElectricity.setReceiptCircuit(receiptCircuitList.get(i - 6));
			dataElectricity.setReadTime(now);
