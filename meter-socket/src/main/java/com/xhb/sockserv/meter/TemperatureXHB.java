...
public class TemperatureXHB extends AbstractDevice {

	private int address;
	private double[] temp = new double[4];

	public TemperatureXHB(ReceiptCollector receiptCollector, ReceiptMeter receiptMeter) {
	...
	public boolean analyzeFrame(byte[] frame) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if(data[2 * i + 3] == 0xFF && data[2 * i + 4] == 0XFF){
				temp[i] = 300;
				continue;
			}
			...
			if (tag == 0x00) {
				temp[i] = (data[2 * i + 3] * 256 + data[2 * i + 4]) / 10.0;
			} else {
				temp[i] = -((data[2 * i + 3] & 0x7F) * 256 + data[2 * i + 4]) / 10.0;
			}
		}
