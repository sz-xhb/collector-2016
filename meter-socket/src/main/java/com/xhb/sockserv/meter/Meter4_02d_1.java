...
import com.xhb.sockserv.meter.Meter4_02;


public class Meter4_02d_1 extends Meter4_02 {

	public Meter4_02d_1(ReceiptCollector receiptCollector, ReceiptMeter receiptMeter) {
	...
	public void handleResult() {
		...
		for (int i = 0; i < 1 && i < receiptCircuitList.size(); i++) {
		...
		for (int i = 3; i < 12 && i - 2 < receiptCircuitList.size(); i++) {
			if (config.isRateFromDataBaseEnabled()) {
				if (receiptCircuitList.get(i-2) == null) {
					return;
				}
				if (receiptCircuitList.get(i-2).getVoltageRatio() != null) {
					Ubb = receiptCircuitList.get(i-2).getVoltageRatio();
				}
				if (receiptCircuitList.get(i-2).getCurrentRatio() != null) {
					Ibb = receiptCircuitList.get(i-2).getCurrentRatio();
				}
			}
			resultMutiplyRate(i);
			DataElectricity dataElectricity = new DataElectricity();
			dataElectricity.setReceiptCircuit(receiptCircuitList.get(i - 2));
			dataElectricity.setReadTime(now);
