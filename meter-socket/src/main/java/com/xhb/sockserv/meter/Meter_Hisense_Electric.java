...
public class Meter_Hisense_Electric extends AbstractDevice {
	...
	public Meter_Hisense_Electric(ReceiptCollector receiptCollector,ReceiptMeter receiptMeter) {
	...
	public boolean analyzeFrame(byte[] frame) {
		...
		voltA = ( data[43] * 256 + data[44] ) * Ubb / 10.0;
		voltB = ( data[45] * 256 + data[46] ) * Ubb / 10.0;
		voltC = ( data[47] * 256 + data[48] ) * Ubb / 10.0;
		voltAB = ( data[49] * 256 + data[50] ) * Ubb / 10.0;
		voltBC = ( data[51] * 256 + data[52] ) * Ubb / 10.0;
		voltCA = ( data[53] * 256 + data[54] ) * Ubb / 10.0;
		currentA = ( data[55] * 256 + data[56] ) * Ibb / 1000.0;
		currentB = ( data[57] * 256 + data[58] ) * Ibb / 1000.0;
		currentC = ( data[59] * 256 + data[60] ) * Ibb / 1000.0;
		bit = data[62];
		kwA = ( data[63] * 256 + data[64] ) * Ubb * Ibb / 10.0 * (((bit & 0x01)== 0x01) ? -1 : 1 );
		kwB = ( data[65] * 256 + data[66] ) * Ubb * Ibb / 10.0 * (((bit & 0x02)== 0x02) ? -1 : 1 );
		kwC = ( data[67] * 256 + data[68] ) * Ubb * Ibb / 10.0 * (((bit & 0x04)== 0x04) ? -1 : 1 );
		kw = ( data[69] * 256 + data[70] ) * Ubb * Ibb/ 10.0 *  (((bit & 0x08)== 0x08) ? -1 : 1 );
		kvarA = ( data[71] * 256 + data[72] )* Ubb * Ibb / 10.0 * (((bit & 0x10)== 0x10) ? -1 : 1 );
		kvarB = ( data[73] * 256 + data[74] )* Ubb * Ibb / 10.0 * (((bit & 0x20)== 0x20) ? -1 : 1 );
		kvarC = ( data[75] * 256 + data[76] )* Ubb * Ibb / 10.0 * (((bit & 0x40)== 0x40) ? -1 : 1 );
		kvar = ( data[77] * 256 + data[78] )* Ubb * Ibb / 10.0  * (((bit & 0x80)== 0x80) ? -1 : 1 );
		kvaA = ( data[79] * 256 + data[80] )* Ubb * Ibb / 10.0;
		kvaB = ( data[81] * 256 + data[82] )* Ubb * Ibb / 10.0;
		kvaC = ( data[83] * 256 + data[84] )* Ubb * Ibb / 10.0;
		kva = ( data[85] * 256 + data[86] )* Ubb * Ibb / 10.0;
